...
import { Search, MoreVertical, MapPin, Users } from "lucide-react"
...
export default function AdminGymsPage() {
  ...
  const handleSuspendGym = (id: string) => {
    setGyms(gyms.map((g) => (g.id === id ? { ...g, status: g.status === "Suspended" ? "Active" : "Suspended" } : g)))
  ...
  return (
    <div className="p-6 space-y-6">
      <div>
        ...
        <p className="text-muted-foreground">Monitor all gym accounts and activity</p>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        {[
          ...
          { label: "Active", value: gyms.filter((g) => g.status === "Active").length },
          { label: "Suspended", value: gyms.filter((g) => g.status === "Suspended").length },
          { label: "Total Members", value: gyms.reduce((sum, g) => sum + g.members, 0) },
        ].map((stat, i) => (
        ...
        <table className="w-full text-sm">
          <thead>
            <tr className="border-b border-border">
              <th className="text-left py-3 px-4 font-semibold text-foreground">Gym Name</th>
              ...
              <tr key={gym.id} className="border-b border-border/50 hover:bg-muted/30">
                ...
                <td className="py-3 px-4">
                  <div className="flex items-center gap-1">
                    ...
                    <span className="font-semibold text-foreground">{gym.members}</span>
                  </div>
                </td>
                <td className="py-3 px-4 text-muted-foreground text-xs">{gym.joinDate}</td>
                <td className="py-3 px-4">
                  <Badge className={statusBadgeColor[gym.status]}>{gym.status}</Badge>
                </td>
                <td className="py-3 px-4">
                  <div className="flex items-center justify-center">
                    <DropdownMenu>
                      ...
                      <DropdownMenuContent align="end">
                        ...
                        <DropdownMenuItem onClick={() => handleSuspendGym(gym.id)}>
                          {gym.status === "Suspended" ? "Reactivate" : "Suspend"}
                        </DropdownMenuItem>
                      </DropdownMenuContent>
                    </DropdownMenu>
                  </div>
