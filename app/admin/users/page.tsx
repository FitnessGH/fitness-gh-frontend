...
export default function AdminUsersPage() {
  ...
  const [filterRole, setFilterRole] = useState<"All" | "Gym Owner" | "Member" | "Vendor" | "Admin">("All")
  ...
  return (
    <div className="p-6 space-y-6">
      <div>
        ...
        <p className="text-muted-foreground">Monitor user accounts and permissions</p>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        {[
          ...
          { label: "Active Users", value: users.filter((u) => u.status === "Active").length, icon: Shield },
          { label: "Suspended", value: users.filter((u) => u.status === "Suspended").length, icon: AlertCircle },
          { label: "Members", value: users.filter((u) => u.role === "Member").length, icon: Shield },
        ].map((stat, i) => {
      ...
      <Card className="p-4 border-border/50">
        <div className="flex flex-col md:flex-row gap-4">
          ...
          <div>
            ...
            <select
              value={filterRole}
              onChange={(e) => setFilterRole(e.target.value as "All" | "Gym Owner" | "Member" | "Vendor" | "Admin")}
        ...
        <table className="w-full text-sm">
          <thead>
            <tr className="border-b border-border">
              ...
              <th className="text-left py-3 px-4 font-semibold text-foreground">Gym</th>
          ...
          <tbody>
            {filteredUsers.map((user) => (
              <tr key={user.id} className="border-b border-border/50 hover:bg-muted/30">
                ...
                <td className="py-3 px-4 text-muted-foreground text-xs">{user.gym || "-"}</td>
