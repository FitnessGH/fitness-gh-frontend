...
import { CheckCircle2, MapPin, Dumbbell, Users, Settings } from "lucide-react"

type OnboardingStep = "gym-info" | "amenities" | "plans" | "employees" | "complete"
...
export default function GymOwnerOnboarding() {
  ...
  const steps: { id: OnboardingStep; label: string; icon: React.ReactNode }[] = [
    ...
    { id: "plans", label: "Membership Plans", icon: <Settings className="w-4 h-4" /> },
  ...
  const handleComplete = () => {
    if (user) {
      // Save gym data to mock backend
      submitGymDetails(user.id, gymData)
      // Update local context/session state
      updateUser({
        approvalStatus: "pending",
        gymDetails: gymData,
      })
      // Logic in layout.tsx will handle the redirect to /gym-owner/approval-pending
    }
  }
    ...
    <div className="min-h-screen bg-gradient-to-br from-background to-secondary/10 p-6">
      <div className="max-w-4xl mx-auto">
        ...
        <div className="text-center mb-8">
          <h1 className="text-3xl font-bold text-primary mb-2">Set Up Your Gym</h1>
          <p className="text-muted-foreground">Complete these steps to launch your gym management dashboard</p>
        </div>
          ...
          {steps.map((step, index) => (
            <React.Fragment key={step.id}>
              <button
                ...
                className={`flex flex-col items-center gap-2 cursor-pointer ${index <= steps.findIndex((s) => s.id === currentStep) ? "opacity-100" : "opacity-50 cursor-default"
                  }`}
              >
                <div
              ...
              {index < steps.length - 1 && (
                <div
                  className={`flex-1 h-1 mx-2 ${index < steps.findIndex((s) => s.id === currentStep) ? "bg-green-500" : "bg-muted"
                    }`}
                />
              )}
            </React.Fragment>
        ...
        <Card className="p-8 border-0 shadow-lg">
          ...
          {currentStep === "gym-info" && (
            <div className="space-y-6">
              <div>
                <h2 className="text-2xl font-bold mb-4 text-foreground">Gym Information</h2>
                <div className="space-y-4">
                  <div>
                    ...
                    <Input
                      ...
                      onChange={(e) => handleGymInfoChange("gymName", e.target.value)}
                    />
                  </div>
                  <div>
                    ...
                    <Input
                      ...
                      onChange={(e) => handleGymInfoChange("location", e.target.value)}
                    />
                  </div>
                  <div className="grid grid-cols-2 gap-4">
                    <div>
                      ...
                      <Input
                        ...
                        value={gymData.phone}
                        onChange={(e) => handleGymInfoChange("phone", e.target.value)}
                      />
                    </div>
                    <div>
                      ...
                      <Input
                        ...
                        value={gymData.email}
                        onChange={(e) => handleGymInfoChange("email", e.target.value)}
                      />
                    </div>
                  </div>
                </div>
          ...
          {currentStep === "amenities" && (
            <div className="space-y-6">
              <div>
                <h2 className="text-2xl font-bold mb-4 text-foreground">Select Amenities</h2>
          ...
          {currentStep === "plans" && (
            <div className="space-y-6">
              <div>
                <h2 className="text-2xl font-bold mb-4 text-foreground">Membership Plans</h2>
                <div className="space-y-4">
                  ...
                  <Button onClick={handleAddPlan} variant="outline" className="w-full bg-transparent">
                    + Add Another Plan
                  </Button>
                </div>
              </div>
          ...
          {currentStep === "employees" && (
            <div className="space-y-6">
              <div>
                <h2 className="text-2xl font-bold mb-4 text-foreground">Add Employees</h2>
                <div className="space-y-4">
                  ...
                  <Button onClick={handleAddEmployee} variant="outline" className="w-full bg-transparent">
          ...
          {currentStep === "complete" && (
            <div className="text-center space-y-6">
              ...
              <div>
                <h2 className="text-2xl font-bold mb-2 text-foreground">All Set!</h2>
                <p className="text-muted-foreground">Your gym is ready to start managing members and operations.</p>
              </div>
              <div className="text-left bg-muted/30 p-4 rounded-lg space-y-2 text-sm">
                ...
                <p>
                  <strong>Location:</strong> {gymData.location || "Not specified"}
                </p>
                <p>
                  <strong>Amenities:</strong> {gymData.amenities.join(", ") || "None selected"}
                </p>
                <p>
                  <strong>Plans:</strong> {gymData.plans.filter((p) => p.name).length} active plan(s)
                </p>
              </div>
            </div>
          )}
          ...
          <div className="flex justify-between mt-8">
            <Button onClick={handlePrevious} variant="outline" disabled={currentStep === "gym-info"}>
              Previous
            </Button>
            {currentStep !== "complete" ? (
              <Button
